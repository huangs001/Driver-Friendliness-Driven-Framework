...
public:
	...
	struct Side{
		...
		std::vector<friend_weight>weight;
...
public:
	...
	ContractionHierarchyQuery&add_source(unsigned s, friend_weight dist_to_s = {0});
	ContractionHierarchyQuery&add_target(unsigned t, friend_weight dist_to_t = {0});
	...
	ContractionHierarchyQuery& get_distances_to_targets(friend_weight*dist);
	...
	ContractionHierarchyQuery& get_distances_to_sources(friend_weight*dist);
