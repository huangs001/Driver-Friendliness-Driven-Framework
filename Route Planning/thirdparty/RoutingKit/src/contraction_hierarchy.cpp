...
namespace{
	...
	// ){
	void sort_arcs_and_remove_multi_and_loop_arcs(
		unsigned node_count, std::vector<unsigned>&tail, std::vector<unsigned>&head, std::vector<friend_weight>&weight, std::vector<unsigned>&input_arc_id,
		const std::function<void(std::string)>&log_message
	){
	...
	public:
		...
		Graph(unsigned node_count, const std::vector<unsigned>&tail, const std::vector<unsigned>&head, const std::vector<friend_weight>&weight):
			out_(node_count),
			in_(node_count),
			level_(node_count, 0){
		...
		void add_arc_or_reduce_arc_weight(unsigned x, unsigned mid_node, unsigned y, friend_weight weight, unsigned hop_length){
			assert(x != y);
...
// Uncomment the following lines to get some very expensive but very detailed asserts
		...
		void assert_no_witness_found(friend_weight len)const{
...
//			}
		...
		void assert_witness_found(friend_weight len)const{
...
//			assert(false && "no witness exists but algorithm claimed that there is one");
		...
		bool forward_settle(
			...
			std::vector<friend_weight>&forward_tentative_distance,
			const std::vector<friend_weight>&backward_tentative_distance,
			...
			friend_weight distance_to_popped_node = p.key;
			...
			for(unsigned out_arc = 0; out_arc < graph_out_deg(popped_node); ++out_arc){
				...
				friend_weight next_node_distance = distance_to_popped_node + graph_out(popped_node, out_arc).weight;
	...
	public:
		...
		bool does_shorter_or_equal_path_to_target_exist(unsigned t, friend_weight len){
		...
		bool does_shorter_or_equal_path_exist(unsigned s, unsigned t, friend_weight len, unsigned bypass){
			if(s == t)
				return true;
	...
	private:
		...
		//std::vector<unsigned>forward_tentative_distance;
		std::vector<friend_weight>forward_tentative_distance;
		std::vector<friend_weight>backward_tentative_distance;
		MinIDQueue forward_queue;
	...
	void build_ch_and_order(
		...
		while(!queue.empty()){
			...
			for(auto x:neighbor_list){
				...
				friend_weight new_key = estimate_node_importance(graph, shorter_path_test, x);
				assert(queue.contains_id(x));
				friend_weight old_key = queue.get_key(x);
				if(old_key < new_key)
...
// ){
ContractionHierarchy ContractionHierarchy::build(
	unsigned node_count, std::vector<unsigned>tail, std::vector<unsigned>head, std::vector<friend_weight>weight,
	const std::function<void(std::string)>&log_message, unsigned max_pop_count
){
...
ContractionHierarchy ContractionHierarchy::build_given_rank(
	std::vector<unsigned>rank,
	std::vector<unsigned>tail, std::vector<unsigned>head, std::vector<friend_weight>weight,
	const std::function<void(std::string)>&log_message, unsigned max_pop_count
){
...
namespace{
	...
	ContractionHierarchy finish_read(std::function<void(char*, unsigned long long)>in, CHFileHeader header){
		...
		ch.forward.weight = read_vector<friend_weight>(in, header.forward_arc_count);
		...
		ch.backward.weight = read_vector<friend_weight>(in, header.backward_arc_count);
...
ContractionHierarchyQuery&ContractionHierarchyQuery::add_source(unsigned external_s, friend_weight dist_to_s){
	assert(ch && "query object must have an attached CH");
...
ContractionHierarchyQuery&ContractionHierarchyQuery::add_target(unsigned external_t, friend_weight dist_to_t){
	assert(ch && "query object must have an attached CH");
...
namespace{
	...
	void forward_expand_upward_ch_arcs_of_node(
		...
		for(unsigned arc = forward_first_out[node]; arc < forward_first_out[node+1]; ++arc){
			unsigned h = forward_head[arc];
			friend_weight d = distance_to_node + forward_weight[arc];
	...
	bool forward_can_stall_at_node(
		unsigned node,
		const std::vector<unsigned>&backward_first_out, const std::vector<unsigned>&backward_head, const std::vector<friend_weight>&backward_weight,
		const TimestampFlags&was_forward_pushed,
		std::vector<friend_weight>&forward_tentative_distance, const std::vector<friend_weight>&backward_tentative_distance
	){
	...
	void forward_settle_node(
		friend_weight&shortest_path_length,
		unsigned&shortest_path_meeting_node,
		const std::vector<unsigned>&forward_first_out, const std::vector<unsigned>&forward_head, const std::vector<friend_weight>&forward_weight,
		const std::vector<unsigned>&backward_first_out, const std::vector<unsigned>&backward_head, const std::vector<friend_weight>&backward_weight,
...
ContractionHierarchyQuery& ContractionHierarchyQuery::run(){
	...
	friend_weight shortest_path_length = inf_weight;
	shortest_path_meeting_node = invalid_id;
...
friend_weight ContractionHierarchyQuery::get_distance() {
	assert(state == ContractionHierarchyQuery::InternalState::run);
...
namespace{
	...
	void pinned_run(
		...
		MinIDQueue&forward_queue,
		std::vector<friend_weight>&tentative_distance,
		...
		for(unsigned i=0; i<select_count; ++i){
			...
			for(unsigned xy = backward_first_out[x]; xy < backward_first_out[x+1]; ++xy){
				...
				friend_weight new_dist = tentative_distance[y]+backward_weight[xy];
				if(new_dist < dist){
					dist = new_dist;
					pred = xy;
				}
	...
	void extract_distances_to_targets(
		...
		unsigned target_count,
		const std::vector<friend_weight>&forward_tentative_distance,
		friend_weight*dist
	){
		for(unsigned i=0; i<target_count; ++i)
	...
	std::vector<friend_weight> extract_distances_to_targets(
		const std::vector<unsigned>&target_list,
		unsigned target_count,
		const std::vector<friend_weight>&forward_tentative_distance
	){
		std::vector<friend_weight>dist(target_count);
...
ContractionHierarchyQuery& ContractionHierarchyQuery::get_distances_to_targets(friend_weight*dist){
...
std::vector<friend_weight> ContractionHierarchyQuery::get_distances_to_targets(){
	assert(state == ContractionHierarchyQuery::InternalState::target_run);
...
ContractionHierarchyQuery& ContractionHierarchyQuery::get_distances_to_sources(friend_weight*dist){
...
std::vector<friend_weight> ContractionHierarchyQuery::get_distances_to_sources(){
	assert(state == ContractionHierarchyQuery::InternalState::source_run);
