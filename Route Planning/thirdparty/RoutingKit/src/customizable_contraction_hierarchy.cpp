...
CustomizableContractionHierarchyMetric::CustomizableContractionHierarchyMetric(const CustomizableContractionHierarchy&cch, const std::vector<friend_weight>&input_weight):
	forward(cch.cch_arc_count()), backward(cch.cch_arc_count()), cch(&cch), input_weight(&input_weight[0]){
}

CustomizableContractionHierarchyMetric::CustomizableContractionHierarchyMetric(const CustomizableContractionHierarchy&cch, const friend_weight*input_weight):
	forward(cch.cch_arc_count()), backward(cch.cch_arc_count()), cch(&cch), input_weight(input_weight){
}

CustomizableContractionHierarchyMetric& CustomizableContractionHierarchyMetric::reset(const CustomizableContractionHierarchy&cch, const std::vector<friend_weight>&input_weight){
	assert(input_weight.size() == cch.input_arc_count() && "Input weight vector has the wrong size");
	reset(cch, &input_weight[0]);
	return *this;
}

CustomizableContractionHierarchyMetric& CustomizableContractionHierarchyMetric::reset(const std::vector<friend_weight>&input_weight){
...
CustomizableContractionHierarchyMetric& CustomizableContractionHierarchyMetric::reset(const CustomizableContractionHierarchy&cch_, const friend_weight*input_weight_){
...
CustomizableContractionHierarchyMetric& CustomizableContractionHierarchyMetric::reset(const friend_weight*input_weight_){
	assert(input_weight_ != nullptr && "Input weight pointer must not be null");
	input_weight = input_weight_;
	return *this;
}
...
namespace{
	template<class T>
	bool my_compare_and_swap(T* ptr, const T& oldval, const T& newval) {
		if (*ptr == oldval) {
			*ptr = newval;
			return true;
		}
	...
	void atomic_min_to(T&x, const T&y){
		 ...
		 while(y < z && !my_compare_and_swap(&x, z, y))
		         z = x;
	}
...
CustomizableContractionHierarchyPartialCustomization&CustomizableContractionHierarchyPartialCustomization::customize(CustomizableContractionHierarchyMetric&metric){
	...
	while(!q.empty()){
		...
		friend_weight old_forward = metric.forward[xy];
		friend_weight old_backward = metric.backward[xy];
		...
		friend_weight new_forward = metric.forward[xy];
		friend_weight new_backward = metric.backward[xy];
...
namespace{
	...
	void reset_source_list(
		...
		std::vector<bool>&in_forward_search_space, std::vector<friend_weight>&forward_tentative_distance
	){
...
namespace{
	void internal_add_source(
		unsigned external_s, friend_weight dist_to_s,
...
namespace{
	...
	void relax_outgoing_arcs(
		...
		const std::vector<friend_weight>&weight,
	...
	for(unsigned i = target_node.size()-1; i!=(unsigned)-1; --i){
		forall_ancestors(
			...
			[&](unsigned x){
				...
				if(in_forward_search_space[x]){
					friend_weight l = forward_tentative_distance[x] + backward_tentative_distance[x];
...
friend_weight CustomizableContractionHierarchyQuery::get_distance(){
...
namespace{
	void internal_run_to_pinned_targets(
		const CustomizableContractionHierarchy&cch,
		const std::vector<friend_weight>&forward_weight, const std::vector<friend_weight>&backward_weight,
		std::vector<friend_weight>&forward_tentative_distance, std::vector<friend_weight>&backward_tentative_distance,
		...
		while(stack_end != 0){
			--stack_end;
			unsigned x = stack[stack_end].travel_time;
...
CustomizableContractionHierarchyQuery& CustomizableContractionHierarchyQuery::get_distances_to_targets(friend_weight*dist){
...
std::vector<friend_weight> CustomizableContractionHierarchyQuery::get_distances_to_targets(){
	assert(state == query_state_target_run);
	std::vector<friend_weight>v(target_node.size());
	get_distances_to_targets(&v[0]);
	return v;
}

CustomizableContractionHierarchyQuery& CustomizableContractionHierarchyQuery::get_distances_to_sources(friend_weight*dist){
...
std::vector<friend_weight> CustomizableContractionHierarchyQuery::get_distances_to_sources(){
	assert(state == query_state_source_run);
	std::vector<friend_weight>v(source_node.size());
