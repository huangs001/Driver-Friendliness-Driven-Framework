...
def stsgcm(data, adj,
    ...
    # shape of each element is (1, 4N, B, C')
    need_concat = [
        mx.sym.expand_dims(
            mx.sym.slice(
                i,
                begin=(0, None, None),
                end=(4 * num_of_vertices, None, None)
            ), 0
        ) for i in need_concat
    ]

    # shape is (4N, B, C')
    tmp_data = mx.sym.max(mx.sym.concat(*need_concat, dim=0), axis=0)
    print("tmp={}".format(tmp_data.infer_shape(data=(32, 16, num_of_vertices, 1))[1]))
...
def sthgcn_layer_individual(data, adj,
    ...
    for i in range(0, T, 4):
        ...
        print("a", t.infer_shape(data=(32, 16, num_of_vertices, 1))[1])
        # shape is (B, 4N, C)
        ...
        print(t.infer_shape(data=(32, 16, num_of_vertices, 1))[1])
        t = mx.sym.slice(t, begin=(j * num_of_vertices, None, None), end=((j + 1) * num_of_vertices, None, None))
        print(t.infer_shape(data=(32, 16, num_of_vertices, 1))[1])
        ...
        #t = mx.sym.reshape(t, (-1, 4, num_of_vertices, num_of_features))
        t = mx.sym.reshape(t, (-1, 1, num_of_vertices, num_of_features))
        print(t.infer_shape(data=(32, 16, num_of_vertices, 1))[1])

        # shape is (B, 1, N, C')
        #need_concat.append(mx.sym.expand_dims(t, axis=1))
        need_concat.append(t)
    print(mx.sym.concat(*need_concat, dim=1).infer_shape(data=(32, 16, num_of_vertices, 1))[1])
    return mx.sym.concat(*need_concat, dim=1)
    # shape is (B, T-1, N, C')
    #data = mx.sym.concat(*need_concat, dim=1)
    #return mx.sym.slice(data, begin=(None, 0, None, None), end=(None, 4 * T, None, None))


def my_data_convert(data, T, num_of_vertices, num_of_features):
    need_concat = []
    for i in range(0, T, 4):
        # shape is (B, 4, N, C)
        t = mx.sym.slice(data, begin=(None, i, None, None),
...
def sthgcn_layer_sharing(data, adj,
    ...
    # shape is (N, T - 0, B, C)
    t = t.reshape((num_of_vertices, T, -1, filters[-1]))
...
def output_layer(data, num_of_vertices, input_length, num_of_features,
    ...
    Parameters
    ----------
    data: mx.sym.var, shape is (B, 4T, N, C)
...
def stsgcn(data, adj, label,
    ...
    #         mx.sym.slice(
    #             i,
    #             begin=(None, None, num_of_vertices, None),
    #             end=(None, None, 2 * num_of_vertices, None)
    #         ), 0
    #     ) for i in need_concat
    # ]

    # data = mx.sym.max(mx.sym.concat(*need_concat, dim=0), axis=0)
    # data = mx.sym.slice(
    #             data,
    #              begin=(None, 0, None, None),
    #              end=(None, input_length, None, None))
    ...
    data = mx.sym.concat(*need_concat, dim=1)
    print("c", data.infer_shape(data=(32, 16, num_of_vertices, 1))[1])
